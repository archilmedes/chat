...
func setupMockUsers() (*User, *User) {
	...
	alice, bob := NewSecureUser("alice"), NewSecureUser("bob")
	alice.IP = "1"
	bob.IP = "2"

	// Read private keys from above
	aliceProto := proto.NewOTRProtocolFromKeys(alicePrivateKey)
	bobProto := proto.NewOTRProtocolFromKeys(bobPrivateKey)
...
func performHandshake(t *testing.T, alice *User, bob *User) {
	...
	for ; initHandshake || len(aMsg[0]) > 0 || len(bMsg[0]) > 0; {
		...
		for _, msg := range aMsg {
			out, err := bob.ReceiveMessage(alice, msg)
			assert.Error(t, proto.OTRHandshakeStep{}, err)
			bMsg = append(bMsg, out)
		}

		aMsg = [][]byte{}
		for _, msg := range bMsg {
			out, err := alice.ReceiveMessage(bob, msg)
			assert.Error(t, proto.OTRHandshakeStep{}, err)
